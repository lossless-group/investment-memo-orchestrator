...
### Outstanding Issues
  ...
  - **RESOLVED (2025-11-21)**: Citation integration issue resolved with new research-first architecture. Citations now generated by Perplexity Sonar Pro BEFORE writing, not retrofitted afterwards. See changelog/2025-11-21_02.md for details.
...
```bash
# 1. Create virtual environment with uv (Python 3.11+)
...
**Troubleshooting:**
- If dependencies disappear, always use `uv pip install -r requirements.txt` (NOT `pip install`)
- Multiple venvs (`.venv`, `venv`) can cause conflicts - use only `.venv`
